...
try:
    ...
    cv2 = None
    logging.warning(
        "OpenCV (cv2) is not installed; video frame analysis is unavailable (%s). "
        "Install dependencies from dlib_video with: pip install -r requirements.txt",
        exc,
    )
import numpy as np
...
LOG_FILE_PATH = Path(tempfile.gettempdir()) / "combined_interview_analysis.log"
...
def main():
    ...
    if args.auto_process and cv2 is None:
        logger.error(
            "Cannot start combined analysis because OpenCV (cv2) is not installed. "
            "Install dependencies from dlib_video with: pip install -r requirements.txt"
        )
        sys.exit(1)
