...
class KafkaFrameClient:
    def __init__(self):
        ...
        # Configuration for both frames and video
        ...
        # Prefer IPv4 for localhost to avoid ::1 issues.
        normalized = []
        for server in bootstrap_servers:
            ...
            if server.startswith("localhost:"):
                normalized.append("127.0.0.1:" + server.split(":", 1)[1])
            normalized.append(server)

        # De-duplicate while preserving order
        seen = set()
        bootstrap_servers = [s for s in normalized if not (s in seen or seen.add(s))]
        ...
        # Try configured bootstrap servers (one at a time for clearer errors)
        server_configs = [[server] for server in bootstrap_servers]
            ...
            try:
                ...
                try:
                    if not self.producer.bootstrap_connected():
                        raise TimeoutError("Kafka bootstrap not connected")
                    logger.info(f"✅ Kafka producer created successfully with {servers}")
                    self._connected = True
                    break
                except Exception as test_error:
                    logger.warning(f"Producer bootstrap test failed: {test_error}")
                    if self.producer:
                        try:
    ...
    def get_video_chunks(self, session_id):
        ...
        try:
            bootstrap_servers = getattr(self, "bootstrap_servers", None) or self.config.get("bootstrap_servers", [])
            if isinstance(bootstrap_servers, str):
                bootstrap_servers = [bootstrap_servers]
            if not bootstrap_servers:
                logger.error("❌ No Kafka bootstrap servers configured")
                return []
