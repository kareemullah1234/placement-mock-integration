...
def login_view(request):
    if request.method == 'POST':
        ...
        # Authenticate using email (mapped to username) for the current AUTH_USER_MODEL
        user_obj = User.objects.filter(email=email).first()
        user = None
        if user_obj is not None:
            user = authenticate(request, username=user_obj.username, password=password)
        if user is None:
            # Fallback: allow logging in with username directly
    ...
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():
                ...
                # Keep PlacementPortal candidate features working.
...
def update_profile(request):
    ...
    try:
        ...
        CandidateProfile.objects.filter(user=user).update(phone=profile.phone_number or "")
...
def get_students_list(request):
    ...
    try:
        ...
        students = User.objects.filter(role='candidate').select_related('student_profile')
        students_data = []
...
def toggle_student_status(request, student_id):
    ...
    try:
        if not _is_admin_user(request.user):
            return JsonResponse({'error': 'Forbidden'}, status=403)
        student = User.objects.get(id=student_id, role='candidate')
        ...
        if user is not None and user.is_active:
            ...
            return JsonResponse({
                ...
                'user': {
                    ...
                    'is_student': getattr(user, 'role', '') == 'candidate',
...
def api_register(request):
    try:
        ...
        with transaction.atomic():
            ...
            CandidateProfile.objects.get_or_create(
                user=user,
                defaults={'phone': data.get('phone_number', '')},
            )
