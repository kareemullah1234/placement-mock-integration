...
if not HAS_SPEECH_RECOGNITION:
    logger.warning(
        "SpeechRecognition is not installed; audio transcription is disabled. "
        "Install dependencies from mock_interview_audio_api/app with: pip install -r requirements.txt"
    )
...
audio_processor = AudioProcessor() if HAS_SPEECH_RECOGNITION else None
...
def analyze_interview(interview_id):
    ...
    try:
        if audio_processor is None:
            return jsonify({
                'error': (
                    "Audio transcription is unavailable because SpeechRecognition is not installed. "
                    "Install dependencies from mock_interview_audio_api/app with: pip install -r requirements.txt"
                )
            }), 503
